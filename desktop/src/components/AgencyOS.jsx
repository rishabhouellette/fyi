...
const AgencyOS = () => {
  ...
  const ClientCard = ({ client, index }) => (
    <motion.div
      ...
      className={`p-6 cursor-pointer relative overflow-hidden rounded-xl border transition-colors ${isDark ? 'bg-gray-900/80 border-cyan-500/20' : 'bg-white border-gray-200 shadow-lg'}`}
    >
      {/* Status Indicator */}
      ...
      <div className="flex items-start gap-4 mb-4">
        <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-cyan-500 to-purple-600 flex items-center justify-center text-3xl">
          {client.avatar}
        </div>

        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h3 className={`font-bold text-lg ${isDark ? 'text-white' : 'text-gray-900'}`}>{client.name}</h3>
            {client.plan === 'premium' && <Crown className="w-4 h-4 text-yellow-500" />}
          </div>
          <div className="flex items-center gap-2">
            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold text-white bg-gradient-to-r ${planColors[client.plan]}`}>
              {client.plan.toUpperCase()}
            </span>
            <span className={`w-2 h-2 rounded-full animate-pulse ${
              client.status === 'active' ? 'bg-green-500' : 'bg-yellow-500'
            }`} />
            <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{client.status}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className={`p-3 rounded-lg ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
          <div className={`text-xs mb-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Revenue</div>
          <div className="text-xl font-bold text-green-500">${client.revenue}</div>
        </div>
        <div className={`p-3 rounded-lg ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
          <div className={`text-xs mb-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Posts</div>
          <div className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{client.posts}</div>
        </div>
        <div className={`p-3 rounded-lg ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
          <div className={`text-xs mb-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Views</div>
          <div className="text-xl font-bold text-cyan-500">{client.views}</div>
        </div>
        <div className={`p-3 rounded-lg ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
          <div className={`text-xs mb-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Engagement</div>
          <div className="text-xl font-bold text-purple-500">{client.engagement}</div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className={`text-4xl font-black mb-2 ${isDark ? 'gradient-text' : 'text-gray-900'}`}>Agency OS</h1>
          <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>Manage all your clients in one powerful dashboard</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Active', value: clients.filter(c => c.status === 'active').length, icon: UserCheck, color: 'from-green-500 to-green-600' },
          { label: 'Monthly Revenue', value: `$${clients.reduce((acc, c) => acc + c.revenue, 0)}`, icon: DollarSign, color: 'from-purple-500 to-purple-600' },
          ...
          <motion.div
            ...
            className={`p-6 relative overflow-hidden rounded-xl border transition-colors ${isDark ? 'bg-gray-900/80 border-cyan-500/20' : 'bg-white border-gray-200 shadow-lg'}`}
          >
            <div className={`absolute inset-0 bg-gradient-to-br ${stat.color} opacity-5`} />
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                <stat.icon className={`w-8 h-8 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
              </div>
              <div className={`text-3xl font-black mb-1 ${isDark ? 'gradient-text' : 'text-cyan-600'}`}>{stat.value}</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Your Clients</h3>
          <div className="flex items-center gap-3">
            <select className={`px-4 py-2 rounded-lg border transition-colors ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-50 border-gray-300 text-gray-900'}`}>
      ...
      >
        <h3 className={`text-xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>Quick Actions</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon: MessageCircle, label: 'Bulk Message', color: 'from-cyan-500 to-blue-600' },
            { icon: Calendar, label: 'Schedule Report', color: 'from-purple-500 to-purple-600' },
            { icon: BarChart3, label: 'Generate Report', color: 'from-green-500 to-green-600' },
